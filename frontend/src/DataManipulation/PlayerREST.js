...
window.onSpotifyWebPlaybackSDKReady = () => {
    ...
    const player = new window.Spotify.Player({
        ...
        player.addListener('player_state_changed', state => { 
            console.log(state); 
            if(state.paused && state.position === 0 && state.restrictions.disallow_resuming_reasons && // finished?
                state.restrictions.disallow_resuming_reasons[0] === "not_paused" && finished){
                finished = false;
                setTimeout(function() { //needs a timeout because two consecutive calls when song finishes
                    finished = true;
                    finishedSong();
                }, 1000);
           }
           ...
           if(state.paused && state.restrictions.disallow_pausing_reasons && state.restrictions.disallow_pausing_reasons[0] === "already_paused") {
...
export const finishedSong = () => { //finished song function calls playNextSong in queue
    if (finished){
...
const playSong = async (uri) => { //should set previous song
    currentSong = uri;
    currentPos = 0;
...
export const loadPlaylist = (playlist, uri) => {// loads playlist to playlistQueue
...
export const resumeSong = async () => { //resumes song
    fetch("https://api.spotify.com/v1/me/player/play?" +
    "device_id=" + deviceID, {
    method: 'PUT',
...
export const playPrevSong = () => { //play previous song
    if (repeat) {
        playSong(currentSong);
    } else {
        if (previousSong == "") {
...
export const repeatSong = () => { //toggle repeat song
    repeat = true;
}

export const noRepeatSong = () => { //toggle repeat song
    repeat = false;
}

export const queueSong = (uri) => { //add songs to the SongQueue
