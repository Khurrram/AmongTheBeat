...
function Admin() {
  ...
  return (
    <CenterDiv className="fullscreen-container">
      ...
      <div>
        <Button onClick={() => setcurrF("Ban User")}>Ban User</Button>
        <Button onClick={() => setcurrF("Unban User")}>Unban User</Button>
        <Button onClick={() => setcurrF("Remove User")}>Remove User</Button>
...
function RB(acc_type, focus, id) {
  //id used to find the user
  if (acc_type !== 1 && focus === "Ban User") {
    //When admin tried to ban an already banned user
    ...
    //Admin tried unbanning a user that was not banned
